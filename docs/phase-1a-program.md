...
**Recommendation: B (Daily recipe).** Recipe = ordered list of activities for today, default `[phonics, spelling, read_aloud]` filtered by the student's active `focus_areas`. Teacher can set per-student recipe length (1–3 items) and order in the dashboard. Default for Ilana's first cohort: all three, in the listed order.
